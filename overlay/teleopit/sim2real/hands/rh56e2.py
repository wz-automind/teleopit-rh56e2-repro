...
def parse_read_response(frame: bytes, *, count: int) -> tuple[int, ...]:
    ...
    if pdu[0] != 0x03 or len(pdu) != expected_bytes + 2 or pdu[1] != expected_bytes:
        raise ModbusProtocolError(
            f"invalid FC03 response: function={pdu[0]:#x}, byte_count={pdu[1] if len(pdu) > 1 else 'missing'}, len={len(pdu)}"
        )
...
def _parse_response(frame: bytes) -> tuple[int, bytes]:
    ...
    # RH56 firmware revisions have been observed to echo the transaction ID
    # incorrectly, so it is deliberately returned for diagnostics, not matched.
